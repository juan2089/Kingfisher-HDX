...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("About", tabName="about_dash",icon=icon("info")),br(),
      column(12,align="center",actionButton(inputId="refresh", label="Clear data")),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),
      div(style="display:inline-block;width:100%;text-align: center; position: absolute; bottom: 2%",socialButton(
    ...
    tabItems(
      ...
      tabItem(tabName="input_dash",
              ...
              bsPopover(id = "testingbutton",title = HTML("<b> Important </b>"),
                        content = paste0("If you are not using an HDExaminer output file, please select the option that fits your data. Check WATERS/Dynamix if you are using a Dinamix output file or check Custom csv for any other type of file. Note: for custom .csv, you will need to manually format your data as in the csv file available for download in the bottom right link."),
                        placement = "right",
                        trigger = "hover",
              ...
              column(4,uiOutput("state1box")),
              column(4,uiOutput("state2box"))
      ),
      ...
      tabItem(tabName="stat_dash",
              fluidPage(shinyjs::useShinyjs(),
                fluidRow(
                  ...
                  column(8,plotOutput(outputId="volcanoplot",width="70%"),align="right"),br(),
                  shinyjs::hidden( div(id = "panel",
                                       column(3,selectInput("colorstr", 
                ...
                br(),
                fluidRow(column(2, uiOutput("volcanotype"),offset=2),
                column(3, uiOutput("volcanodownload"),offset=1),
      ...
      tabItem(tabName="bayesian_dash",
              fluidPage(shinyjs::hidden(div(id="functional",
               ...
               tags$iframe(src='./functionalanalysis.html',
                           ...
                           frameborder = 0, scrolling = "no"),
               column(12, radioButtons(inputId = "selectedeq", label = h3("Select the desired equation to fit the data:"), inline = TRUE,
                                       choiceNames = list(
               ...
               fluidRow(
                 column(3,
                        numericInput("a_input", 
                                     label=HTML("Parameter a:"), 
                                     value = 0,min=0,max=100)), 
                 column(3,
                        numericInput("b_input", 
                                     label=HTML("Parameter b:"), 
                                     value = 0,min=0,max=100)), 
                 column(3,
                        numericInput("d_input", 
                                     label=HTML("Parameter d:"), 
                                     value = 0,min=0,max=100)),
                 column(3,
                        numericInput("p_input", 
                                     label=HTML("Parameter p:"), 
                                     value = 1,min=0,max=1)) 
                 
               ),br(),
               actionButton("startfitting","Calculate"))),br(),
               shinyjs::hidden(div(id="fdabox",
                      ...
                      tabPanel("Fitted kinetic plots", "Additional description for kinetic plots",
                               ...
                               column(8,align="center",selectInput(inputId="selpeptide2",label = "Select peptide:",choices =""),
                                       offset=2),br(),br(),
                               fluidRow(column(3, selectInput("kinetictype", 
                                                              label = NULL,
                                                              choices = c(".pdf", 
                                                                          ".svg",
                                                                          ".eps"
                                                              ),
                                                              selected = ".pdf"),offset=2
                               ),column(3,downloadButton("kineticbutton", "Download kinetic plot")),column(3,actionButton("downloadallkinetic","Download all kinetic plots",icon=icon("download")),align="center"))),
                      tabPanel("Manhattan plot","Additional description for Manhattan plots",br(),
                               column(3,selectInput(inputId="seltimemanhattan",label = "Select labeling time:",choices =""),offset=4,align="center"),br(),
                               fluidRow(
      ...
      tabItem(tabName="globwoods_dash",
              fluidPage(
                fluidRow(
                  ...
                  column(8,plotOutput(outputId="globwoodsplot",width="70%"),align="right"),
                  column(3,br(),shinyjs::hidden(div(id="panel2",selectInput("colorflex", 
      ...
      tabItem(tabName="pep_dash",
              fluidPage(
                ...
                column(12,shinyjs::hidden(div(id="pep",
                        selectInput("colorpep", 
                            ...
                            selected = "blue")
                      ,align="center"))),br(),
                shinyjs::hidden(div(id="pep2",
                                   h3("Summary peptide map"),br(),plotOutput(outputId="summaryplot",width="70%",height="200px"),br(),
                                    downloadButton("summarypepbutton", "Download summary plot"),align="center"
                                    ))
              )
      ),
      #7th tab
      tabItem(tabName="uptake_plotsub",
              fluidPage(
                fluidRow(
                  column(3,shinyjs::hidden(div(id="plotssettings",
                         ...
                         shinyjs::hidden(div(id="limits",
                                             fluidRow(
                           column(6,numericInput("yminvalue",label="Y-axis min",value ="",min=0,max=25)),
                           column(6,numericInput("ymaxvalue",label="Y-axis max",value ="",min=0,max=25))       
                           
                         )
                         )) 
                         ))),
                  column(9,plotOutput(outputId="uptakeplot",width="80%"),align="center")),
                br(),
                column(3,uiOutput("uptaketype"),offset=4),
                column(2,uiOutput("singledownload",offset=4)),
                column(2,uiOutput("uptakedownload")),
                br(),
                column(12,shinyjs::hidden(div(id="allplotscheck",
                                              checkboxInput(inputId="showallstates","Check to show all protein states",value=FALSE),align="center"))),
                uiOutput("uptaketext")
              )
      ),
      #8th tab
      tabItem(tabName="3d_dash",
              ...
              fluidRow(shinyjs::hidden(div(id="3dtoggles", column(1,uiOutput("spinbox")),column(3,materialSwitch("clustercoloring",label="Color using k-means clustering results",value=FALSE,status = "success"))))),
